...
-- high_scores has no unique constraint, so a plain ON CONFLICT DO NOTHING
-- would re-insert these on every startup; only seed an empty table
...
SELECT * FROM (VALUES
  ...
  ('Anonymous', 75, 4),
  ('ProGamer', 100, 5)
) AS seed(player_name, score, obstacles_avoided)
